...
with lib;
let
  cfg = config.services.santiment.sanbase;
  user = "www";
  group = "www";
  rootFolder = "${cfg.package}/public";
  uid = 1666;
  gid = 1666;
...
in
{
  ...
  options.services.santiment.sanbase = {
    enable = mkOption {
    ...
    package = mkOption {
      description = "Default sanbase package";
      default = pkgs.santiment.sanbase;
      type = types.package;
    };
